...
userdskchk()
{
    UDS=`/bin/ls -sl /images/vol0* | /bin/grep $C_USER | /usr/bin/awk '{x=x+$6} END {printf "%.0f\n",x/1024/1024/1024}'`
    UDU=`/bin/ls -sl /images/vol0* | /bin/grep $C_USER | /usr/bin/awk '{x=x+$1} END {printf "%.0f\n",x/1024/1024}'`
}
...
AAS=`/bin/ls -sl /images/vol0*/* | /usr/bin/awk '{x=x+$6} END {printf "%.0f\n",x/1024/1024/1024}'`
AUS=`/bin/ls -sl /images/vol0*/* | /usr/bin/awk '{x=x+$1} END {printf "%.0f\n",x/1024/1024}'`
